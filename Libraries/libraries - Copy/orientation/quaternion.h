...
struct Quat {
    ...
    Vec quatToVec(){
      ...
      v.x = atan2(2.0f * i * j - 2.0f * w * k,
                    2.0f * w * w + 2.0f * i * i - 1.0f);
      auto check = 2.0f * i * k + 2.0f * w * j;
      if (abs(check) >= 1.0f) {
          v.y = -sk_math::SIGN(check) * PI / 2.0f;
      } else {
          v.y = -asin(check);
      }
      v.z = atan2(2.0f * j * k - 2.0f * w * i,
                    2.0f * w * w + 2.0f * k * k - 1.0f);
      return v;
    }

    // Angular rate must be given in rad/s
    Quat fromAngularRate(Vec v) {
    ...
    Quat conjugate() { return Quat(w, -i, -j, -k); }
